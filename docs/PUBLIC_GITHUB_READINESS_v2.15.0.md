...
Current deployment instructions are only:
...
Both must pass from the exact files intended for upload.

Immediately before publication also inspect the Git staging area manually for unexpected files. A verifier cannot detect every possible piece of contextual student information.
